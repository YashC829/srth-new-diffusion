...
class CameraRegistrationApplication:
    def __init__(self, ral, args, arm_name, marker_size, expected_interval, camera):
        self.ral = ral
        self.args = args
        self.camera = camera
    ...
    def collect_poses_manually(self):
        ...
        while True:
            user_input = input("Press Enter to collect data, or type 'q' to stop: ")
            if user_input.lower() == 'q':
                print("Exiting loop.")
                break
            else:
                # Simulate data collection
                current_pose = self.arm.measured_jp()
                poses.append(current_pose)
                print(f"Collected data: {current_pose}")
                print(f"Total data collected: {len(poses)}")

        # Optionally do something with the collected data
        print(str(len(poses)) + " points have been collected")

        return poses
    
    def collect_poses_autonomously(self):
        
        print("move the tool around, it will pick up points autonomously, when done, press d")
        idx = 0
        ...
        while self.ok and not self.done:
            ...
            # current_position_local = self.arm.local.forward_kinematics(jp)
            # current_position = self.arm.forward_kinematics(jp)
            # print("jp", jp)
            # print("current_position_local", current_position_local)
            # print("current_position", current_position)
            position = np.array([pose[0], pose[1], pose[2]])
            if len(poses) == 0 or distance(position, 
                                           np.array((poses[-1][0], 
                                                     poses[-1][1], 
                                                     poses[-1][2]))) > 0.005:
    ...
    def collect_data_simple(self, poses):
        ...
        def collect(poses, tool_shaft_rotation=math.pi / 8):
            ...
            for i, pose in enumerate(poses):
                ...
                tool_shaft_rotation = random.uniform(0, math.pi / 5)
                ...
                self.messages.progress((i + 1) / len(poses))
    ...
    def run(self):
        try:
            ...
            if self.args.manual_collection:
                # collect points manually
                poses = self.collect_poses_manually()

                # collect data
                data = self.collect_data_simple(poses)
            
            elif self.args.autonomous_collection:
                poses = self.collect_poses_autonomously()
                data = self.collect_data_simple(poses)
...
def main():
    ...
    parser.add_argument('--manual_collection', 
                        action='store_true', 
                        help='Whether the calibration points will be specified manually')
    
    parser.add_argument('--autonomous_collection', 
                        action='store_true', 
                        help='Whether the calibration points will be specified manually')
