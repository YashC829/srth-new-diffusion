...
class PoseAnnotator:
    ...
    def draw_pose_on_img(self, img: numpy.ndarray, local_measured_cp: numpy.ndarray):
        ...
        xx = 0.027
        yy=  -0.001#-0.009#-0.003
        zz = 0.002#0.003
        translation_vector = numpy.array([zz, yy, xx])  # Replace x, y, z with the desired translation values

        # Create the 4x4 homogeneous transformation matrix
        offset = numpy.eye(4)  # Start with a 4x4 identity matrix
        offset[:3, 3] = translation_vector  # Set the translation part
