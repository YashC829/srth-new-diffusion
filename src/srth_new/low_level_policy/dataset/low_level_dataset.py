...
class EpisodicDatasetDvrkGeneric(torch.utils.data.Dataset):
    ...
    def preprocess_img(self, img, start_ts):
        ...
        # Convert Color and Type
        img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img_rgb = cv2.resize(img, (self.img_width, self.img_height), interpolation=cv2.INTER_LINEAR)
        
        # Convert to tensor and change HWC -> CHW 
        # (Using .permute is slightly more standard for this than einsum)
        img_tensor = torch.from_numpy(img_rgb).int().permute(2, 0, 1)
    ...
    def __getitem__(self, index):
        ...
        for retry_attempt in range(max_retries):
            try:
                ...
                # -------------------------------
                # 3. Change images to CHW, RGB, tensors
                # -------------------------------
                ...
                tfmed, endo_cam_spatial_tf_only = self.transforms(img_dict, episode_path=dataset_path, phase_config=phase_config)
                ...
                endo_cam_spatial_tf_only = endo_cam_spatial_tf_only / 255.0
                ...
                return image_data, current_pose_data, action_data, is_pad, command_text, endo_cam_spatial_tf_only
...
if __name__ == "__main__":
    ...
    img_stack, current_pose_data, action_data, is_pad, command_text, endo_spatial_tf_only = train_dataset[0]

    endo = (img_stack[0].cpu() * 255).to(torch.uint8)
    lw = (img_stack[1].cpu() * 255).to(torch.uint8)
    rw = (img_stack[2].cpu() * 255).to(torch.uint8)

    from PIL import Image

    def save_img(img, save_name: str):
        img = (img * 255).to(torch.uint8)
        img = img.permute(1, 2, 0).cpu().numpy()
        Image.fromarray(img).save(save_name)

    save_img(endo, "endo.png")
    save_img(lw, "lw.png")
    save_img(rw, "rw.png")
    save_img(endo_spatial_tf_only, "endo_spatial_tf_only.png")
...
#                 return img_dict["left"], img_dict["left_wrist"], img_dict["right_wrist"], current_pose_data, action_data, is_pad, command_text
...
#         use_auto_label
#     )
#     endoscope_img, lw_img, rw_img, current_pose_data, action_data, is_pad, command_text = train_dataset[0]
#     pass