...
class EpisodicDatasetDvrkGeneric(torch.utils.data.Dataset):
    ...
    def preprocess_img(self, img, start_ts):
        ...
        img_tensor = torch.from_numpy(img_rgb).to(torch.uint8).permute(2, 0, 1)
    ...
    def __getitem__(self, index):
        ...
        for retry_attempt in range(max_retries):
            try:
                ...
                # This just outputs synced raw images...
...
if __name__ == "__main__":
    ...
    endoscope_img, lw_img, rw_img, current_pose_data, action_data, is_pad, command_text = next(iter(train_dataloader))



    Augmentor = EpisodicDatasetDvrkGenericDataAug()

    endoscope_img_aug = Augmentor(endoscope_img)
    ...
    temp_save_dir = "dataset_debug"
    os.makedirs(temp_save_dir, exist_ok=True)
    for idx in range(endoscope_img.shape[0]):
        save_img(endoscope_img[idx], os.path.join(temp_save_dir, f"endo_before_{idx}.png"))
        save_img(endoscope_img_aug[idx], os.path.join(temp_save_dir, f"endo_after_{idx}.png"))