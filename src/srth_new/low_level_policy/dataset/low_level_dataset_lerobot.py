...
class EpisodicDatasetDvrkGeneric(torch.utils.data.Dataset):
    def __init__(
        ...
        delta_timestamps = {
            "action": [t / self.ds_meta.fps for t in range(chunk_size)],
        }

        ds_lerobot = LeRobotDataset(repo_id, delta_timestamps=delta_timestamps)

        # filter by phase and tissue id
        high_level_phases = [dataset_utils.clean_high_level_phase_from_cfg(x) for x in get_high_level_phases_from_phase_cfg(phases)]
        low_level_phases = [dataset_utils.clean_low_level_phase_from_cfg(x) for x in get_low_level_phases_from_phase_cfg(phases)]
...
if __name__=="__main__":
    ...
    Image.fromarray(convert(endo_img).cpu().numpy().transpose(1, 2, 0)).save("endo_img.png")
