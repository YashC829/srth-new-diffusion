...
from .resnet_film import film_resnet18, film_resnet34, film_resnet50
...
class FilMedBackbone(torch.nn.Module):
    ...
    def __init__(self, name: str, average_pool: bool = False):
        super().__init__()
        # Load pretrained weights.
        if name == "resnet18film":
            weights = ResNet18_Weights.DEFAULT
            self.num_channels = 512
            self.backbone = film_resnet18(weights=None, use_film=True)
    ...
    def forward(self, img_obs, language_embed):
        ...
        # print(img_obs.shape)
        # tmp = img_obs.permute(0, 2, 3, 1)
        # tmp_img = tmp[0].cpu().numpy()
        # plt.imshow(tmp_img)
        # plt.savefig(f"/home/marin/visualization.png")
        out = self.backbone(
